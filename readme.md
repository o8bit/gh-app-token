...
`o8bit/cloud-builders` holds custom build-step container images, published so
consuming repositories do not each carry their own copy of the same script.

| Builder | Image |
|---|---|
| [`gh-app-token`](gh-app-token/) | `ghcr.io/o8bit/gh-app-token` |

These usage instructions live in the **root** README rather than in
`gh-app-token/`, because GitHub Container Registry renders a package's linked
repository root README on the package landing page and cannot be pointed at a
subdirectory. Anyone arriving from the package page needs to find the
instructions there.

### Adding a builder

Create a directory with a `Dockerfile` and a workflow under
...
When a second builder is added, this arrangement stops working — two packages
cannot both render their docs from one root README. At that point give each
builder its own repository so each package page shows its own instructions.
